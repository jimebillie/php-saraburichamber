...
if (!isset($_POST["name"]) || !isset($_POST['id'])) {
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["name"] !== "") {
        ...
        if ($type_file[0] !== "image") {
            ...
            header("Location: ../spare_parts_sub_edit.php?id_sub=" . $_POST["id"] . "#image");
            exit;
        } else {
            // Valid image
             ...
             */
            try {
                $stmt = $db->conn->prepare("UPDATE `spare_parts_sub` SET `name`=:__name,`image`=:__image WHERE `id`=:__id");
             ...
             * Seccessful
             */
            $_SESSION["resp_pass"] = "Edit spare_parts-sub already ";
            header("Location: ../spare_parts_sub_edit.php?id_sub=" . $_POST["id"]);
            exit;
        }
    } else {
        //-> Not have
         ...
         */
        try {
            $stmt = $db->conn->prepare("UPDATE `spare_parts_sub` SET `name`=:__name WHERE `id`=:__id");
