...
if (!isset($_FILES["aboutus_image"])) {
    ...
    if ($type_file[0] !== "image") {
        ...
        $db_count = 0;
        try {
            $stmt = $db->conn->prepare("SELECT COUNT(id) as _count FROM `aboutus_image`");
            $stmt->execute();
            $result = $stmt->fetch();
        ...
        if ($db_count <= 0) {
            ...
            $newName = date("YmdHis") . "-aboutus" . $file_extension;
            /**
             * Move file upload
             */
            try {
             ...
             */
            try {
                $stmt = $db->conn->prepare("INSERT INTO `aboutus_image`(`img`) VALUES (:img)");
            ...
            try {
                ...
                if (file_exists("../images/aboutus/" . $result["img"])) {
                     ...
                     */
                    try {
                        $stmt = $db->conn->prepare("UPDATE `aboutus_image` SET `img`=:_img WHERE `id`=:_id");
